...
import { 
  ...
  MapPin, Home, IndianRupee, ImageIcon, UploadCloud, CheckCircle2 
} from "lucide-react";
...
export default function EditListing() {
  ...
  // --- UI STATES ---
  const [fetching, setFetching] = useState(true); // Fixes the crash in Screenshot 419
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        const { data } = await api.get(`/pg-listings/${id}`);
        ...
        setForm({
          ...
          availableRooms: l.rooms.availableRooms,
          roomType: l.rooms.roomType,
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // Logic from Screenshot 420: Limit to 5 photos total
  ...
  const onUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/pg-listings/${id}`, formData);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4 animate-in fade-in duration-500">
      <header className="flex justify-between items-center mb-10">
        ...
        <Button onClick={onUpdate} disabled={loading} className="bg-emerald-600 text-white rounded-2xl px-10 h-14 font-bold shadow-lg shadow-emerald-100">
      ...
      <form onSubmit={onUpdate} className="space-y-10">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold italic">Photos</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            {existingPhotos.map((img, i) => (
              <div key={`old-${i}`} className="relative aspect-square rounded-3xl overflow-hidden border border-gray-100">
                <img src={img.url} className="h-full w-full object-cover opacity-60" />
            ...
            {newPhotos.map((file, i) => (
              <div key={`new-${i}`} className="relative aspect-square rounded-3xl overflow-hidden border-2 border-emerald-400 group">
                <img src={URL.createObjectURL(file)} className="h-full w-full object-cover" />
                <button type="button" onClick={() => setNewPhotos(prev => prev.filter((_, idx) => idx !== i))} className="absolute top-2 right-2 p-1 bg-white rounded-full text-red-500">
                  <X size={12}/>
                </button>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
            ...
            <Input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="h-14 rounded-2xl bg-gray-50 border-none font-bold" placeholder="PG Name" />
            <Textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="rounded-2xl bg-gray-50 border-none font-medium min-h-[120px]" placeholder="Description" />
          </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <Input placeholder="Add custom tag..." value={newAmenity} onChange={e => setNewAmenity(e.target.value)} className="h-10 rounded-xl bg-gray-50 border-none text-xs font-bold" />
              <Button type="button" onClick={() => {
                if(newAmenity) setForm({...form, amenities: [...form.amenities, newAmenity.toLowerCase().trim()]});
                setNewAmenity("");
              }} className="h-10 w-10 bg-gray-900 rounded-xl"><Plus/></Button>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="relative">
              <Input type="number" value={form.pricePerMonth} onChange={e => setForm({...form, pricePerMonth: e.target.value})} className="h-14 rounded-2xl bg-gray-50 border-none font-black text-2xl" />
              <IndianRupee className="absolute right-4 top-4 text-gray-300" size={20} />
            </div>
            <select value={form.roomType} onChange={e => setForm({...form, roomType: e.target.value})} className="h-14 rounded-2xl bg-gray-50 border-none px-4 font-bold outline-none">
              <option value="single">Single Sharing</option>
              <option value="double">Double Sharing</option>
              <option value="triple">Triple Sharing</option>
            </select>
            <Input type="number" value={form.availableRooms} onChange={e => setForm({...form, availableRooms: e.target.value})} className="h-14 rounded-2xl bg-gray-50 border-none font-bold" placeholder="Rooms" />
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-50">
            {form.amenities.map(a => (
              <div key={a} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100 text-[10px] font-black uppercase">
                <Sparkles size={12}/> {a}
                <X size={14} className="cursor-pointer" onClick={() => setForm(prev => ({...prev, amenities: prev.amenities.filter(i => i !== a)}))} />
              </div>
            ))}
          </div>
        </div>
